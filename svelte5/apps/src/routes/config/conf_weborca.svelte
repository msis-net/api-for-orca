<script lang="ts">
	...
	let cert = $state<string | null>(null);
	onMount(() => {
	...
	$effect(() => {
		if (Orca.orcaCertificate) {
			...
			cert = Orca.orcaCertificate;
			orcaCertName.removeAttribute('for');
			orcaCertName.style.display = 'flex';
			orcaCertName.style.setProperty('--after-content', '"[X]"');
			// 文字列を省略する処理
			...
			// スタイルの追加は1回だけ
			if (!document.head.querySelector('#orcaCertStyle')) {
				const style = document.createElement('style');
				style.id = 'orcaCertStyle';
				style.textContent = `
					#orcaCertName::after {
			...
			style.textContent = `
				#orcaCertificate::file-selector-button {
					content: var(--file-selector-button-content);
				}
			`;
			document.head.appendChild(style);
		}
	...
	const certFileHandler = async (event: Event) => {
		...
		if (target.files) {
			...
			try {
				...
				//const orcaCertName = document.getElementById('orcaCertName') as HTMLInputElement;
				//orcaCertName.addEventListener('click', certFileHandler);
			} catch (error) {
	...
	const certFileDeleteHandler = async (event: Event) => {
		...
		try {
			...
			target.textContent = '証明書(.p12)を選択してください。';
			target.style.setProperty('--after-content', '');
